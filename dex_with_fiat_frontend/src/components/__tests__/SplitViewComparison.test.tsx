...
describe('SplitViewComparison – interactions', () => {
  ...
  it('optimistically swaps panes before parent state catches up', () => {
    fireEvent.click(screen.getByTestId('swap-threads-btn'));

    expect(splitView.swapSessions).toHaveBeenCalledOnce();
    expect(
      within(screen.getByTestId('split-pane-left')).getByText(
        'Hello from thread B',
      ),
    ).toBeDefined();
    expect(
      within(screen.getByTestId('split-pane-right')).getByText(
        'Hello from thread A',
      ),
    ).toBeDefined();
    expect(screen.getByRole('status')).toHaveTextContent('Updating comparison');
  });

  it('optimistically changes a pane session before parent state catches up', () => {
    fireEvent.change(screen.getByLabelText('Select Left thread'), {
      target: { value: 's2' },
    });

    expect(splitView.setLeftSession).toHaveBeenCalledWith('s2');
    expect(
      within(screen.getByTestId('split-pane-left')).getByText(
        'Hello from thread B',
      ),
...
describe('SplitViewComparison – message selection sync', () => {
  ...
  it('optimistically marks a clicked message as selected before parent state catches up', () => {
    const splitView = makeSplitView();
    render(
      <SplitViewComparison splitView={splitView} sessions={allSessions} />,
    );
...
describe('SplitViewComparison – network status toasts', () => {
  ...
  it('renders timestamps after the comparison panes mount', async () => {
    ...
    await waitFor(() => {
      ...
      timestampElements.forEach((el) => {
        expect(el.textContent).toMatch(/Jun 15/);
      });
    });
  });
});
  ...
  it('detects offline→online transition even when events fire in rapid succession', async () => {
    ...
    await waitFor(() => {
      ...
      const severities = calls.map(
        (c: [{ severity: string }]) => c[0].severity,
      );
