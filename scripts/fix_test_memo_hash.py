...
f = pathlib.Path(
    "/Users/mac/Desktop/CODE/stellar-dex-chat/stellar-contracts/src/test.rs"
)
src = f.read_text()
...
src = re.sub(r"(\.request_withdrawal\(&\w+,\s*&\d+,\s*&\w+)\)", r"\1, &None)", src)

# --- Fix 5: try_request_withdrawal with 3 user args ---
src = re.sub(r"(\.try_request_withdrawal\(&\w+,\s*&\d+,\s*&\w+)\)", r"\1, &None)", src)
