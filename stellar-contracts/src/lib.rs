...
impl FiatBridge {
    pub fn init(env: Env, admin: Address, token: Address, limit: i128) -> Result<(), Error> {
        ...
        let config = TokenConfig { limit, total_deposited: 0 };
        env.storage().persistent().set(&DataKey::TokenRegistry(token), &config);
    ...
    pub fn execute_withdrawal(env: Env, request_id: u64, partial_amount: Option<i128>) -> Result<(), Error> {
        env.storage().instance().extend_ttl(MIN_TTL, MAX_TTL);
        let mut request: WithdrawRequest = env.storage().persistent().get(&DataKey::WithdrawQueue(request_id))
    ...
    pub fn cancel_withdrawal(env: Env, request_id: u64) -> Result<(), Error> {
        let admin: Address = env.storage().instance().get(&DataKey::Admin).ok_or(Error::NotInitialized)?;
        admin.require_auth();
        if !env.storage().persistent().has(&DataKey::WithdrawQueue(request_id)) {
    ...
    pub fn set_limit(env: Env, token: Address, limit: i128) -> Result<(), Error> {
        let admin: Address = env.storage().instance().get(&DataKey::Admin).ok_or(Error::NotInitialized)?;
        admin.require_auth();
        if limit <= 0 { return Err(Error::ZeroAmount); }
        let mut config: TokenConfig = env.storage().persistent().get(&DataKey::TokenRegistry(token.clone()))
            .ok_or(Error::TokenNotWhitelisted)?;
        config.limit = limit;
        env.storage().persistent().set(&DataKey::TokenRegistry(token), &config);
    ...
    pub fn set_fiat_limit(env: Env, limit_usd_cents: i128) -> Result<(), Error> {
        let admin: Address = env.storage().instance().get(&DataKey::Admin).ok_or(Error::NotInitialized)?;
        admin.require_auth();
        if limit_usd_cents <= 0 { return Err(Error::ZeroAmount); }
        env.storage().instance().set(&DataKey::FiatLimit, &limit_usd_cents);
        Ok(())
    }
    ...
    fn validate_fiat_limit(env: &Env, depositor: &Address, token: &Address, amount: i128) -> Result<(), Error> {
        ...
        if vol.usd_cents + usd_cents > fiat_limit { return Err(Error::ExceedsFiatLimit); }
        vol.usd_cents += usd_cents;
        env.storage().instance().set(&DataKey::UserDailyVolume(depositor.clone()), &vol);
        Ok(())
    }

    // ── Timelock ──────────────────────────────────────────────────────────
    pub fn queue_admin_action(env: Env, action_type: Symbol, payload: Bytes, delay: u32) -> Result<u64, Error> {
        let admin: Address = env.storage().instance().get(&DataKey::Admin).ok_or(Error::NotInitialized)?;
        admin.require_auth();
    ...
    pub fn execute_admin_action(env: Env, id: u64) -> Result<(), Error> {
        ...
        if env.ledger().sequence() <= action.target_ledger { return Err(Error::ActionNotReady); }
        env.storage().persistent().remove(&DataKey::QueuedAdminAction(id));
        env.storage().instance().set(&DataKey::LastAdminActionLedger, &env.ledger().sequence());
        Ok(())
    }
    ...
    pub fn get_total_deposited(env: Env) -> i128 {
        let tok = env.storage().instance().get::<_, Address>(&DataKey::Token).unwrap();
        env.storage().persistent().get::<_, TokenConfig>(&DataKey::TokenRegistry(tok)).unwrap().total_deposited
    }
    ...
    pub fn get_pending_admin(env: Env) -> Option<Address> { env.storage().instance().get(&DataKey::PendingAdmin) }
    ...
    pub fn get_receipt(env: Env, id: u64) -> Option<Receipt> { env.storage().persistent().get(&DataKey::Receipt(id)) }
    pub fn get_receipt_counter(env: Env) -> u64 { env.storage().instance().get(&DataKey::ReceiptCounter).unwrap_or(0) }
    ...
    pub fn deposit_for(env: Env, payer: Address, beneficiary: Address, amount: i128, token: Address, reference: Bytes) -> Result<u64, Error> {
        payer.require_auth();
        if amount <= 0 { return Err(Error::ZeroAmount); }
        if reference.len() > 64 { return Err(Error::ReferenceTooLong); }
        let cooldown: u32 = env.storage().instance().get(&DataKey::CooldownLedgers).unwrap_or(0);
        if cooldown > 0 {
            let key = DataKey::LastDeposit(beneficiary.clone());
            if let Some(last) = env.storage().temporary().get::<DataKey, u32>(&key) {
                if env.ledger().sequence() < last + cooldown { return Err(Error::CooldownActive); }
            }
        }
        let mut config: TokenConfig = env.storage().persistent().get(&DataKey::TokenRegistry(token.clone()))
            .ok_or(Error::TokenNotWhitelisted)?;
        ...
        let prev: i128 = env.storage().instance().get(&user_key).unwrap_or(0);
        env.storage().instance().set(&user_key, &(prev + amount));
        let receipt_id: u64 = env.storage().instance().get(&DataKey::ReceiptCounter).unwrap_or(0);
        let receipt = Receipt { id: receipt_id, depositor: beneficiary.clone(), amount, ledger: env.ledger().sequence(), reference, refunded: false };
        env.storage().persistent().set(&DataKey::Receipt(receipt_id), &receipt);
        env.storage().instance().set(&DataKey::ReceiptCounter, &(receipt_id + 1));
        if cooldown > 0 {
    ...
    pub fn emergency_drain(env: Env, recipient: Address) -> Result<(), Error> {
        ...
        let balance = soroban_sdk::token::Client::new(&env, &tok).balance(&env.current_contract_address());
        if balance <= 0 { return Err(Error::ZeroAmount); }
        soroban_sdk::token::Client::new(&env, &tok).transfer(&env.current_contract_address(), &recipient, &balance);
        Ok(())
    }
}
